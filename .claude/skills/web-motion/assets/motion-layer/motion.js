/* ============================================================================
   Motion layer — orchestration.

   The only file that imports GSAP. Pages stay declarative: they mark up intent
   with data-attributes and never touch an animation API.

   Contract with motion.css: if initMotion() never runs, the inline <head>
   timeout removes `html.js` and everything pre-hidden reappears.
   ========================================================================== */
...
/* ---------------------------------------------------------------------------
   ...
   Three.js is ~120 KB gzip and purely decorative, so it gets its own chunk and
   is not even requested unless the device can afford to give it away. The gate
   runs BEFORE the dynamic import — that is the whole point.
   ------------------------------------------------------------------------- */
...
async function heroField() {
  const mount = document.querySelector("[data-hero-field]");
  ...
  try {
    const { mountField } = await import("./hero-field.js");
    fieldCleanup = mountField(mount);
  } catch {
    // No WebGL, or the chunk failed. The hero is identical, just quieter.
    mount.remove();
  }
}
